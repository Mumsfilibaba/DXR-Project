...
FRHIResource::FRHIResource()
    : StrongReferences(1)
    , AliveState(static_cast<int32>(EState::Alive))
{
}
...
FRHIResource::~FRHIResource()
{
    ...
    CHECK(AliveState.Load() == static_cast<int32>(EState::Deleted));
}

int32 FRHIResource::AddRef() const
{
    CHECK(StrongReferences.Load() > 0);
    CHECK(AliveState.Load() == static_cast<int32>(EState::Alive));
    ++StrongReferences;
...
int32 FRHIResource::Release() const
{
    ...
    if (RefCount < 1)
    {
        AliveState = static_cast<int32>(EState::Deleted);
