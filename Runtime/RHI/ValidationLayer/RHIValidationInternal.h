...
namespace RHIValidationInternal
{
    ...
    bool     ValidateBufferRange(const TCHAR* Caller, const FRHIBufferDesc& BufferDesc, uint64 Offset, uint64 Size);
    bool     ValidateIndirectCountBuffer(const CHAR* Operation, FRHIBuffer* CountBuffer, uint64 CountBufferOffset);
    bool     ValidateBufferView(const TCHAR* Caller, const FRHIBufferDesc& BufferDesc, EBufferViewType ViewType, uint32 FirstElement, uint32 NumElements, EFormat Format);
    bool     ValidateTextureMip(const TCHAR* Caller, const FRHITextureDesc& TextureDesc, uint32 MipLevel, IntVector3& OutExtent);
    bool     ValidateTextureRegion2D(const TCHAR* Caller, const FRHITextureDesc& TextureDesc, uint32 MipLevel, const FTextureRegion2D& Region);
    bool     ValidateTextureRegion3D(const TCHAR* Caller, const FRHITextureDesc& TextureDesc, uint32 MipLevel, const FTextureRegion3D& Region);
    bool     ValidateTextureSlicesAndMips(const TCHAR* Caller, const FRHITextureDesc& TextureDesc, uint32 BaseLayer, uint32 LayerCount, uint32 FirstMip, uint32 NumMips, EFormat ViewFormat, EViewDimension ViewDimension);
}
