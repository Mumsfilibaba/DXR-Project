...
class ENGINE_API FMeshComponent : public FRendererComponent
{
public:
    FOBJECT_DECLARE_CLASS(FMeshComponent, FRendererComponent);

    FMeshComponent(const FObjectInitializer& ObjectInitializer);
    ~FMeshComponent() = default;

    virtual FProxyRendererComponent* CreateProxyComponent() override final;
