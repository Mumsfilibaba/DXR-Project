...
void FVulkanCommandContext::ClearUnorderedAccessViewFloat(FRHIUnorderedAccessView* UnorderedAccessView, const Vector4& ClearColor)
{
    ...
    else if (Type == FVulkanResourceView::EType::StructuredBufferView)
    {
        ...
        RequireBufferState(VulkanUnorderedAccessView, EResourceAccess::CopyDest);
        BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::UpdateBuffer(FRHIBuffer* Dst, const FBufferRegion& BufferRegion, const void* SrcData)     
{
    ...
    else
    {
        ...
        RequireBufferState(VulkanBuffer, EResourceAccess::CopyDest);
        BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::CopyBuffer(FRHIBuffer* Dst, FRHIBuffer* Src, const FRHIBufferCopyDesc& CopyDesc)
{
    ...
    RequireBufferState(SrcVulkanBuffer, EResourceAccess::CopySource);
    RequireBufferState(DstVulkanBuffer, EResourceAccess::CopyDest);
    BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::SerializeAccelerationStructure(FRHIRayTracingAccelerationStructure* Source, FRHIBuffer* DstBuffer, uint64 DstOffset)
{
    ...
    RequireBufferState(VulkanDestination, EResourceAccess::CopyDest | EResourceAccess::RayTracingAccelerationStructure);
    BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::DeserializeAccelerationStructure(FRHIRayTracingAccelerationStructure* Destination, FRHIBuffer* SourceBuffer, uint64 SourceOffset)
{
    ...
    RequireBufferState(VulkanSource, EResourceAccess::CopySource | EResourceAccess::RayTracingAccelerationStructure);
    BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::WriteAccelerationStructurePostBuildInfo(FRHIBuffer* DstBuffer, uint64 DstOffset, EAccelerationStructurePostBuildInfoType InfoType, FRHIRayTracingAccelerationStructure* const* Sources, uint32 NumSources)
{
    ...
    AddAccelerationStructureMemoryBarrier();
    RequireBufferState(VulkanDestination, EResourceAccess::CopyDest);
    BarrierBatcher.FlushBarriers(GetCommandBuffer());
...
void FVulkanCommandContext::DispatchRaysIndirect(FRHIShaderBindingTable* ShaderBindingTable, FRHIBuffer* ArgumentBuffer, uint64 ArgumentBufferOffset)
{
    ...
    RequireBufferState(VulkanArgumentBuffer, EResourceAccess::IndirectArgument);
...
void FVulkanCommandContext::TransitionTextureState(FRHITexture* Texture, const FRHITextureTransition& TextureTransition)
{
    ...
    const VkImageLayout      DeclaredLayout = FVulkanDeviceRHI::ResourceStateToImageLayout(TextureTransition.BeforeState);
    const VkImageCreateInfo& CreateInfo     = VulkanTexture->GetVkImageCreateInfo();
    ...
    const bool bWholeResource = (TextureTransition.MipLevel == RHI_ALL_MIP_LEVELS) && (TextureTransition.ArraySlice == RHI_ALL_ARRAY_SLICES);
    if (bWholeResource && LocalState.AreAllSubresourcesSameLayout())
    {
        ...
        if (CurrentLayout == VK_IMAGE_LAYOUT_TO_BE_DETERMINED)
        {
            ...
            PendingBarrier.DesiredLayout = DeclaredLayout;
            PendingBarrier.Subresource   = RHI_ALL_MIP_LEVELS;
            PendingImageBarriers.Add(PendingBarrier);
        }

        const VkImageLayout OldLayout = (CurrentLayout == VK_IMAGE_LAYOUT_TO_BE_DETERMINED) ? DeclaredLayout : CurrentLayout;
...
void FVulkanCommandContext::TransitionBufferState(FRHIBuffer* Buffer, EResourceAccess BeforeState, EResourceAccess AfterState)   
{
    ...
    const VkAccessFlags2        DeclaredAccess = FVulkanDeviceRHI::ResourceStateToAccessFlags(BeforeState);
    const VkPipelineStageFlags2 DeclaredStage  = FVulkanDeviceRHI::ResourceStateToPipelineStageFlags(BeforeState);

    const bool bIsTracked = LocalState.GetAccess() != VK_ACCESS_FLAGS_2_TO_BE_DETERMINED;
    if (!bIsTracked)
    {
    ...
    const VkAccessFlags2        BeforeAccess = bIsTracked ? LocalState.GetAccess() : DeclaredAccess;
    const VkPipelineStageFlags2 BeforeStage  = bIsTracked ? LocalState.GetStage()  : DeclaredStage;
...
{
    VkMemoryBarrier2 AccelerationStructureBarrier = {};
    AccelerationStructureBarrier.sType         = VK_STRUCTURE_TYPE_MEMORY_BARRIER_2;
    AccelerationStructureBarrier.srcStageMask  = VK_PIPELINE_STAGE_2_HOST_BIT | VK_PIPELINE_STAGE_2_TRANSFER_BIT | VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR;
    AccelerationStructureBarrier.srcAccessMask = VK_ACCESS_2_HOST_WRITE_BIT | VK_ACCESS_2_TRANSFER_WRITE_BIT | VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;
    AccelerationStructureBarrier.dstStageMask  = VK_PIPELINE_STAGE_2_ACCELERATION_STRUCTURE_BUILD_BIT_KHR;
    AccelerationStructureBarrier.dstAccessMask = VK_ACCESS_2_SHADER_READ_BIT | VK_ACCESS_2_TRANSFER_READ_BIT | VK_ACCESS_2_TRANSFER_WRITE_BIT | VK_ACCESS_2_ACCELERATION_STRUCTURE_READ_BIT_KHR | VK_ACCESS_2_ACCELERATION_STRUCTURE_WRITE_BIT_KHR;

    BarrierBatcher.AddMemoryBarrier(0, AccelerationStructureBarrier);
}

void FVulkanCommandContext::RequireBufferState(FVulkanUnorderedAccessViewRHI* View, EResourceAccess RequiredState)
{
    CHECK(View != nullptr);
...
void FVulkanCommandContext::DrawIndirect(FRHIBuffer* ArgumentBuffer, uint64 ArgumentBufferOffset, uint32 CommandCount)
{
    ...
    RequireBufferState(Arguments, EResourceAccess::IndirectArgument);
    ContextState.PrepareGraphicsState();
...
void FVulkanCommandContext::DrawIndirectCount(FRHIBuffer* ArgumentBuffer, uint64 ArgumentBufferOffset, FRHIBuffer* CountBuffer, uint64 CountBufferOffset, uint32 MaxCommandCount)
{
    ...
    RequireBufferState(Arguments, EResourceAccess::IndirectArgument);
    RequireBufferState(Count, EResourceAccess::IndirectArgument);
    ContextState.PrepareGraphicsState();
