...
bool CRenderer::Init()
{
    ...
    Resources.CameraBuffer = RHICreateConstantBuffer<SCameraBufferDesc>(BufferFlag_Default, EResourceAccess::Common, nullptr);
...
void CRenderer::PerformAABBDebugPass(CRHICommandList& InCmdList)
{
    ...
    InCmdList.SetIndexBuffer(AABBIndexBuffer.Get(), EIndexFormat::uint32);
...
bool CRenderer::InitBoundingBoxDebugPass()
{
    ...
    PSOProperties.PrimitiveTopologyType                  = EPrimitiveTopology::Line;
    ...
    AABBVertexBuffer = RHICreateVertexBuffer<CVector3>(Vertices.Size(), BufferFlag_Default, EResourceAccess::Common, &VertexData);
    if (!AABBVertexBuffer)
    {
        CDebug::DebugBreak();
        return false;
    ...
    AABBIndexBuffer = RHICreateIndexBuffer(EIndexFormat::uint16, Indices.Size(), BufferFlag_Default, EResourceAccess::Common, &IndexData);
    if (!AABBIndexBuffer)
    {
        CDebug::DebugBreak();
        return false;
...
bool CRenderer::InitShadingImage()
{
    ...
    ShadingImage = RHICreateTexture2D(ERHIFormat::R8_Uint, Width, Height, 1, 1, TextureFlags_RWTexture, EResourceAccess::ShadingRateSource, nullptr);
    if (!ShadingImage)
    {
        CDebug::DebugBreak();
        return false;
