...
FSceneSkyLight::FSceneSkyLight(FSkyLight* InLight)
    : Light(InLight)
    ...
    , DiffuseCubeMap(nullptr)
{
    if (InLight)
    {
        SourceCubeMap = InLight->GetCubeMap();
    }
}
...
void FSceneSkyLight::FilterStaticCubeMaps()
{
    ...
    FRHITextureRef TempDiffuseCubeMap = RHICreateTexture(DiffuseCubeMapInfo, EResourceAccess::PixelShaderResource);
