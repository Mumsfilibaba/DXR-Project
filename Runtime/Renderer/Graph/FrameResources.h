...
struct FFrameResources
{
    ...
    {
        return bReflectionHalfRes ? Math::Max<uint32>((ReflectionFullWidth + 1u) / 2u, 1u) : ReflectionFullWidth;
    }

    NODISCARD uint32 GetReflectionChainHeight() const
    {
        return bReflectionHalfRes ? Math::Max<uint32>((ReflectionFullHeight + 1u) / 2u, 1u) : ReflectionFullHeight;
    }
...
#if EDITOR_BUILD // Editor-only: non-jittered depth + ObjectID buffers (used for stable selection outlines and picking).
    ...
    bool   bReflectionHalfRes   = false;
    uint32 ReflectionFullWidth  = 0;
    uint32 ReflectionFullHeight = 0;
