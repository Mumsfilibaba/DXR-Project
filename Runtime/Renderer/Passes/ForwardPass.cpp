...
#include "Renderer/Settings/ShadingSettings.h"
...
static FAutoConsoleVariableRef CVarForwardPassBindless(
    ...
    "When true, the forward pass samples per-material textures (Albedo / Normal / Material / Height) and the material sampler via SM 6.6 "
    "ResourceDescriptorHeap[] / SamplerDescriptorHeap[] and a per-material indices buffer instead of register bindings. Full-frame SRVs / "
    "samplers (sky, integration LUT, shadow maps) remain non-bindless.",
...
FGraphicsPipelineStateInstance* FForwardPass::CompilePipelineState(const FMaterialFeatures& Features, bool bBindless, const FVertexDeclaration& Declaration)
{
    ...
    if (bTranslucent)
    {
        ...
        BlendStateDesc.RenderTargets[0].ColorWriteMask =
            EColorWriteFlags::Red |
            EColorWriteFlags::Green |
            EColorWriteFlags::Blue;
    }
    ...
    const String DebugName = String::Printf("ForwardPass PipelineState%s%s%s%s%s%s [Declaration %u]",
        Permutation.Get<FParallax>() ? " [Parallax]" : "", Permutation.Get<FClipping>() ? " [Clipping]" : "",
        Permutation.Get<FAlphaMask>() ? " [AlphaMask]" : "", Permutation.Get<FTranslucent>() ? " [Translucent]" : "",
        Permutation.Get<FRefraction>() ? " [Refraction]" : "", Permutation.Get<FBindless>() ? " [Bindless]" : "",
...
void FForwardPass::Record(FRHICommandList& CommandList, const FFrameResources& FrameResources, FScene* Scene)
{
    // Forward Pass
    ...
    FForwardPassConstants ForwardConstants;

    const FSceneDirectionalLight* DirectionalLight = Scene ? Scene->GetDirectionalLight() : nullptr;
    const bool bEnableSunShadows =
        GShadowsEnabled &&
        GSunShadowsEnabled &&
        DirectionalLight &&
        DirectionalLight->bCastShadows &&
        FrameResources.ShadowCascades;
...
void FForwardPass::AddRenderGraphPass(FRenderGraphBuilder& GraphBuilder, const FSceneRenderGraphContext& Context)
{
    GraphBuilder.AddPass("ForwardPass", ERenderGraphPassFlags::Raster, GForwardPassEnabled,
        [&Context](FRenderGraphPassBuilder& PassBuilder)
        {
            PassBuilder.SetRenderTarget(0, Context.SceneTargetRenderTargetView, EAttachmentLoadAction::Load);
            ...
            PassBuilder.ReadTexture(Context.ShadowCascades, ERHIResourceState::PixelShaderResource);
            PassBuilder.ReadTexture(Context.PointLightShadowMaps, ERHIResourceState::PixelShaderResource);

            if (Context.IntegrationLUT)
            {
                PassBuilder.ReadTexture(Context.IntegrationLUT, ERHIResourceState::PixelShaderResource);
            }

            if (Context.CascadeSplitsBufferSRV)
            {
                PassBuilder.Read(Context.CascadeSplitsBufferSRV, ERHIResourceState::PixelShaderResource);
            }
        },
