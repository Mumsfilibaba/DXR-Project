#pragma once
#include "RHI/RHICommandList.h"
#include "RHI/RHIShader.h"
#include "Engine/World/World.h"
#include "Renderer/Graph/PassResources.h"
#include "Renderer/Passes/RenderPass.h"
#include "Renderer/Graph/FrameResources.h"
#include "Renderer/Graph/SceneRenderGraphContext.h"

namespace HaltonPrivate
{
    constexpr float RadicalInverse2(uint32 Bits)
    {
        Bits = (Bits << 16u) | (Bits >> 16u);
        Bits = ((Bits & 0x55555555u) << 1u) | ((Bits & 0xAAAAAAAAu) >> 1u);
        Bits = ((Bits & 0x33333333u) << 2u) | ((Bits & 0xCCCCCCCCu) >> 2u);
        Bits = ((Bits & 0x0F0F0F0Fu) << 4u) | ((Bits & 0xF0F0F0F0u) >> 4u);
        Bits = ((Bits & 0x00FF00FFu) << 8u) | ((Bits & 0xFF00FF00u) >> 8u);
        return float(double(Bits) * 2.3283064365386963e-10);
    }

    constexpr float RadicalInverse3(uint32 Sample)
    {
        constexpr float OneMinusEpsilon = 0x1.fffffep-1;

        const uint32 Base = 3;
        const float InvBase = 1.0f / float(Base);

        uint32 ReversedDigits = 0;
        float InvBaseN = 1.0f;

        while (Sample)
        {
            const uint32 Next  = Sample / Base;
            const uint32 Digit = Sample - Next * Base;
            ReversedDigits = ReversedDigits * Base + Digit;
            InvBaseN *= InvBase;
            Sample = Next;
        }

        return Math::Min<float>(float(ReversedDigits) * InvBaseN, OneMinusEpsilon);
    }

    inline Vector2 Halton23(uint32 Sample)
    {
        return Vector2(RadicalInverse2(Sample), RadicalInverse3(Sample));
    }
}

struct FHaltonState
{
    FHaltonState(const uint32 InMaxNumSamples = 16)
        : MaxNumSamples(InMaxNumSamples)
        , SampleIndex(0)
    {
    }

    Vector2 NextSample()
    {
        SampleIndex = (SampleIndex + 1) % MaxNumSamples;
        Vector2 Sample = HaltonPrivate::Halton23(SampleIndex);
        return (Sample * 2.0f) - 1.0f;
    }

    const uint32 MaxNumSamples;
    uint32       SampleIndex;
};

class FRenderGraphBuilder;

class FTemporalAntiAliasing : public FRenderPass
{
public:
    FTemporalAntiAliasing(FSceneRenderer* InRenderer);
    virtual ~FTemporalAntiAliasing();

    bool Initialize(FFrameResources& FrameResources);
    bool CreateResources(FFrameResources& FrameResources, uint32 Width, uint32 Height);
    void AddRenderGraphPass(FRenderGraphBuilder& GraphBuilder, const FSceneRenderGraphContext& Context);

    void InvalidateHistory()
    {
        bHistoryValid = false;
    }

    NODISCARD FRHITexture* GetHistoryBuffer(uint32 Index) const
    {
        return TAAHistoryBuffers[Index].Get();
    }

private:
    void Record(FRHICommandList& CommandList, FFrameResources& FrameResources, uint32 ReadIndex, uint32 WriteIndex);
    void RecordHistorySeed(FRHICommandList& CommandList, FFrameResources& FrameResources);

    FRHIComputePipelineStateRef TemporalAntiAliasingPSO;
    FRHIComputeShaderRef        TemporalAntiAliasingShader;
    FRHITextureRef              TAAHistoryBuffers[2];
    FRHISamplerStateRef         LinearSampler;
    uint32                      CurrentBufferIndex;
    bool                        bHistoryValid;
};
