...
void FD3D12CommandContext::ClearRenderTargetView(const FRHIRenderTargetView& RenderTargetView, const FVector4& ClearColor)
{
    ...
    FD3D12Texture* D3D12Texture = FD3D12RHI::ResourceCast(RenderTargetView.Texture);
...
void FD3D12CommandContext::ClearDepthStencilView(const FRHIDepthStencilView& DepthStencilView, const float Depth, uint8 Stencil)
{
    ...
    FD3D12Texture* D3D12Texture = FD3D12RHI::ResourceCast(DepthStencilView.Texture);
    CHECK(D3D12Texture != nullptr);
    ...
    if (FormatHasStencil(DepthStencilView.Format))
    {
...
{
    FD3D12UnorderedAccessView* D3D12UnorderedAccessView = FD3D12RHI::ResourceCast(UnorderedAccessView);
...
void FD3D12CommandContext::BeginRenderPass(const FRHIBeginRenderPassInfo& BeginRenderPassInfo)
{
    ...
    for (uint32 Index = 0; Index < BeginRenderPassInfo.NumRenderTargets; ++Index)
    {
        ...
        if (FD3D12Texture* RenderTarget = FD3D12RHI::ResourceCast(CurrentRTV.Texture))
        {
    ...
    if (FD3D12Texture* DepthStencil = FD3D12RHI::ResourceCast(CurrentDSV.Texture))
    {
        ...
        {
            D3D12_CLEAR_FLAGS ClearFlags = D3D12_CLEAR_FLAG_DEPTH;
            if (FormatHasStencil(CurrentDSV.Format))
            {
                ClearFlags |= D3D12_CLEAR_FLAG_STENCIL;
            }
            ...
            GetCommandList()->ClearDepthStencilView(
                ...
                ClearFlags,
                CurrentDSV.ClearValue.Depth,
                static_cast<uint8>(CurrentDSV.ClearValue.Stencil),
                0,
                nullptr);
        }
    ...
    // ShadingRate
    FD3D12Texture* ShadingRateImage = FD3D12RHI::ResourceCast(BeginRenderPassInfo.ShadingRateTexture);
...
void FD3D12CommandContext::SetVertexBuffers(const TArrayView<FRHIBuffer* const> InVertexBuffers, uint32 BufferSlot)
{
    ...
    {
        FD3D12Buffer* D3DVertexBuffer = FD3D12RHI::ResourceCast(InVertexBuffers[Index]);
...
{
    FD3D12Buffer* D3DIndexBuffer = FD3D12RHI::ResourceCast(IndexBuffer);
...
{
    FD3D12GraphicsPipelineState* GraphicsPipelineState = FD3D12RHI::ResourceCast(PipelineState);
...
{
    FD3D12ComputePipelineState* ComputePipelineState = FD3D12RHI::ResourceCast(PipelineState);
...
void FD3D12CommandContext::SetShaderResourceView(FRHIShader* Shader, FRHIShaderResourceView* ShaderResourceView, uint32 RegisterIndex)
{
    ...
    FD3D12ShaderResourceView* D3D12ShaderResourceView = FD3D12RHI::ResourceCast(ShaderResourceView);
...
void FD3D12CommandContext::SetShaderResourceViews(FRHIShader* Shader, const TArrayView<FRHIShaderResourceView* const> InShaderResourceViews, uint32 RegisterIndex)
{
    ...
    {
        FD3D12ShaderResourceView* D3D12ShaderResourceView = FD3D12RHI::ResourceCast(InShaderResourceViews[Index]);
...
void FD3D12CommandContext::SetUnorderedAccessViews(FRHIShader* Shader, const TArrayView<FRHIUnorderedAccessView* const> InUnorderedAccessViews, uint32 RegisterIndex)
{
    ...
    {
        FD3D12UnorderedAccessView* D3D12UnorderedAccessView = FD3D12RHI::ResourceCast(InUnorderedAccessViews[Index]);
...
void FD3D12CommandContext::SetConstantBuffer(FRHIShader* Shader, FRHIBuffer* ConstantBuffer, uint32 RegisterIndex)
{
    ...
    FD3D12Buffer* D3D12Buffer = ConstantBuffer ? FD3D12RHI::ResourceCast(ConstantBuffer) : nullptr;
...
void FD3D12CommandContext::SetConstantBuffers(FRHIShader* Shader, const TArrayView<FRHIBuffer* const> InConstantBuffers, uint32 RegisterIndex)
{
    ...
    {
        FD3D12Buffer* D3D12Buffer = InConstantBuffers[Index] ? FD3D12RHI::ResourceCast(InConstantBuffers[Index]) : nullptr;
...
void FD3D12CommandContext::SetSamplerState(FRHIShader* Shader, FRHISamplerState* SamplerState, uint32 RegisterIndex)
{
    ...
    FD3D12SamplerState* D3D12SamplerState = FD3D12RHI::ResourceCast(SamplerState);
...
void FD3D12CommandContext::SetSamplerStates(FRHIShader* Shader, const TArrayView<FRHISamplerState* const> InSamplerStates, uint32 RegisterIndex)
{
    ...
    {
        FD3D12SamplerState* D3D12SamplerState = FD3D12RHI::ResourceCast(InSamplerStates[Index]);
...
void FD3D12CommandContext::ResolveTexture(FRHITexture* Dst, FRHITexture* Src)
{
    ...
    FD3D12Texture* D3D12Source      = FD3D12RHI::ResourceCast(Src);
    FD3D12Texture* D3D12Destination = FD3D12RHI::ResourceCast(Dst);
...
void FD3D12CommandContext::UpdateBuffer(FRHIBuffer* Dst, const FBufferRegion& BufferRegion, const void* SrcData)
{
    ...
    FD3D12Buffer* D3D12Destination = FD3D12RHI::ResourceCast(Dst);
...
void FD3D12CommandContext::CopyBuffer(FRHIBuffer* Dst, FRHIBuffer* Src, const FBufferCopyInfo& CopyInfo)
{
    ...
    FD3D12Buffer* D3D12Source = FD3D12RHI::ResourceCast(Src);
    CHECK(D3D12Source != nullptr);

    GetCommandList().UpdateResidency(D3D12Destination->GetResource()->GetResidencyHandle());
    GetCommandList().UpdateResidency(D3D12Source->GetResource()->GetResidencyHandle());

    const uint64 DstOffset = CopyInfo.DstOffset + D3D12Destination->GetResourceStorage().GetResourceOffset();
    const uint64 SrcOffset = CopyInfo.SrcOffset + D3D12Source->GetResourceStorage().GetResourceOffset();
...
void FD3D12CommandContext::CopyTextureRegion(FRHITexture* Dst, FRHITexture* Src, const FTextureCopyInfo& InCopyDesc)
{
    ...
    for (uint32 ArraySlice = 0; ArraySlice < NumArraySlices; ArraySlice++)
    {
        for (uint32 MipLevel = 0; MipLevel < InCopyDesc.NumMipLevels; MipLevel++)
        {
            ...
            SourceLocation.SubresourceIndex = D3D12CalculateSubresource(
            ...
            DestLocation.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;
            DestLocation.SubresourceIndex = D3D12CalculateSubresource(
...
void FD3D12CommandContext::WriteFence(FRHIGpuFence* Fence)
{
    ...
    FD3D12GpuFence* D3D12Fence = FD3D12RHI::ResourceCast(Fence);
...
{
    if (FD3D12Texture* D3D12Texture = FD3D12RHI::ResourceCast(Texture))
    {
...
void FD3D12CommandContext::BuildRayTracingScene(FRHIRayTracingScene* RayTracingScene, const FRayTracingSceneBuildInfo& BuildInfo)
{
    ...
    FD3D12RayTracingScene* D3D12RayTracingScene = FD3D12RHI::ResourceCast(RayTracingScene);
...
void FD3D12CommandContext::BuildRayTracingGeometry(FRHIRayTracingGeometry* RayTracingGeometry, const FRayTracingGeometryBuildInfo& BuildInfo)
{
    ...
    FD3D12RayTracingGeometry* D3D12RayTracingGeometry = FD3D12RHI::ResourceCast(RayTracingGeometry);
...
{
#if 0
    FD3D12RayTracingScene* D3D12Scene = FD3D12RHI::ResourceCast(RayTracingScene);
    D3D12_ERROR_COND(D3D12Scene != nullptr, "RayTracingScene cannot be nullptr");

    FD3D12RayTracingPipelineState* D3D12PipelineState = FD3D12RHI::ResourceCast(PipelineState);
    ...
    if (GlobalResource)
    {
        if (!GlobalResource->ConstantBuffers.IsEmpty())
        {
            ...
            {
                FD3D12Buffer* D3D12Buffer = FD3D12RHI::ResourceCast(GlobalResource->ConstantBuffers[i]);
        ...
        if (!GlobalResource->ShaderResourceViews.IsEmpty())
        {
            ...
            {
                FD3D12ShaderResourceView* D3D12ShaderResourceView = FD3D12RHI::ResourceCast(GlobalResource->ShaderResourceViews[i]);
        ...
        if (!GlobalResource->UnorderedAccessViews.IsEmpty())
        {
            ...
            {
                FD3D12UnorderedAccessView* D3D12UnorderedAccessView = FD3D12RHI::ResourceCast(GlobalResource->UnorderedAccessViews[i]);
        ...
        if (!GlobalResource->SamplerStates.IsEmpty())
        {
            ...
            {
                FD3D12SamplerState* DxSampler = FD3D12RHI::ResourceCast(GlobalResource->SamplerStates[i]);
...
{
    FD3D12Texture* D3D12Texture = FD3D12RHI::ResourceCast(Texture);
    CHECK(D3D12Texture != nullptr);
...
{
    FD3D12Buffer* D3D12Buffer = FD3D12RHI::ResourceCast(Buffer);
...
void FD3D12CommandContext::TransitionResourceState(FD3D12UnorderedAccessView* View)
{
    ...
    const D3D12_UNORDERED_ACCESS_VIEW_DESC& ViewDesc = View->GetDesc();
    switch (ViewDesc.ViewDimension)
    {
        ...
        {
            TransitionResourceState(Resource, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, ViewDesc.Texture2DArray.MipSlice, 1, ViewDesc.Texture2DArray.FirstArraySlice, ViewDesc.Texture2DArray.ArraySize);
            break;
        }
...
void FD3D12CommandContext::TransitionResourceState(FD3D12ShaderResourceView* View, D3D12_RESOURCE_STATES State)
{
    FD3D12Resource* Resource = View->GetViewResource();
    if (!Resource || !Resource->RequiresResourceStateTracking())
    {
    ...
    const D3D12_RESOURCE_DESC&             ResourceDesc = Resource->GetDesc();
    const D3D12_SHADER_RESOURCE_VIEW_DESC& ViewDesc     = View->GetDesc();
    ...
    switch (ViewDesc.ViewDimension)
    {
        ...
        case D3D12_SRV_DIMENSION_TEXTURECUBE:
        {
            ...
            TransitionResourceState(Resource, State, ViewDesc.TextureCube.MostDetailedMip, NumMips, 0, 6);
            break;
        }
        ...
        case D3D12_SRV_DIMENSION_TEXTURECUBEARRAY:
        {
            ...
            TransitionResourceState(Resource, State, ViewDesc.TextureCubeArray.MostDetailedMip, NumMips, ViewDesc.TextureCubeArray.First2DArrayFace, ViewDesc.TextureCubeArray.NumCubes * 6);
            break;
        }
...
void FD3D12CommandContext::TransitionResourceState(FD3D12RenderTargetView* View)
{
    FD3D12Resource* Resource = View->GetViewResource();
    if (!Resource || !Resource->RequiresResourceStateTracking())
    {
    ...
    const D3D12_RENDER_TARGET_VIEW_DESC& ViewDesc = View->GetDesc();
    switch (ViewDesc.ViewDimension)
    {
        ...
        {
            TransitionResourceState(Resource, D3D12_RESOURCE_STATE_RENDER_TARGET, ViewDesc.Texture2DArray.MipSlice, 1, ViewDesc.Texture2DArray.FirstArraySlice, ViewDesc.Texture2DArray.ArraySize);
            break;
        }
        ...
        {
            TransitionResourceState(Resource, D3D12_RESOURCE_STATE_RENDER_TARGET, 0, 1, ViewDesc.Texture2DMSArray.FirstArraySlice, ViewDesc.Texture2DMSArray.ArraySize);
            break;
        }
...
void FD3D12CommandContext::TransitionResourceState(FD3D12DepthStencilView* View, D3D12_RESOURCE_STATES State)
{
    FD3D12Resource* Resource = View->GetViewResource();
    if (!Resource || !Resource->RequiresResourceStateTracking())
    {
    ...
    const D3D12_DEPTH_STENCIL_VIEW_DESC& ViewDesc = View->GetDesc();
    switch (ViewDesc.ViewDimension)
    {
...
void FD3D12CommandContext::PresentSwapChain(FRHISwapChain* SwapChain, bool bVerticalSync)
{
    ...
    FD3D12SwapChain* D3D12SwapChain = FD3D12RHI::ResourceCast(SwapChain);
