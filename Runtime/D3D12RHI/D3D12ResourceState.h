...
FORCEINLINE D3D12_RESOURCE_STATES D3D12GetAllowedReadStates(ED3D12CommandQueueType QueueType)
{
    // Compute and copy queues reject PIXEL_SHADER_RESOURCE, so the shader-read mask has to be narrowed for them.
    return (QueueType == ED3D12CommandQueueType::Direct) ? GD3D12CombinableReadStates : D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
}

inline constexpr D3D12_RESOURCE_STATES GD3D12ReadOnlyStates =
...
{
    return State != D3D12_RESOURCE_STATES(0) && (State & ~GD3D12ReadOnlyStates) == D3D12_RESOURCE_STATES(0);
}
