...
void FImGuiRenderer::PrepareDrawData(FRHICommandList& CommandList, ImDrawData* DrawData)
{
    ...
    if (!ViewportData->VertexBuffer || DrawData->TotalVtxCount > ViewportData->VertexCount)
    {
        ...
        VBDesc.Stride = sizeof(ImDrawVert);
        VBDesc.Size   = VBDesc.Stride * NewVertexCount;
        VBDesc.Flags  = EBufferFlags::VertexBuffer | EBufferFlags::Default;

        TSharedRef<FRHIBuffer> NewVertexBuffer = FRHI::Get()->CreateBuffer(VBDesc, EResourceAccess::GenericRead, nullptr);
        if (NewVertexBuffer)
        {
    ...
    if (!ViewportData->IndexBuffer || DrawData->TotalIdxCount > ViewportData->IndexCount)
    {
        ...
        IBDesc.Stride = sizeof(ImDrawIdx);
        IBDesc.Size   = IBDesc.Stride * NewIndexCount;
        IBDesc.Flags  = EBufferFlags::IndexBuffer | EBufferFlags::Default;

        TSharedRef<FRHIBuffer> NewIndexBuffer = FRHI::Get()->CreateBuffer(IBDesc, EResourceAccess::GenericRead, nullptr);
        if (NewIndexBuffer)
        {
