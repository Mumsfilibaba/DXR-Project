...
bool D3D12RayTracingGeometry::BuildAccelerationStructure(D3D12CommandList* CommandList, TSharedRef<D3D12VertexBuffer>& InVertexBuffer, Uint32 InVertexCount, TSharedRef<D3D12IndexBuffer>& InIndexBuffer, Uint32 InIndexCount)
{
	if (!IsDirty)
	{
		return true;
	...
	Memory::Memzero(&Inputs, sizeof(D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO));
	...
	ScratchBuffer = static_cast<D3D12StructuredBuffer*>(
		RenderingAPI::CreateStructuredBuffer(nullptr, Info.ScratchDataSizeInBytes, 1, BufferUsage_UAV | BufferUsage_Default));
	if (!ScratchBuffer)
	{
		return false;
	}
	...
	ResultBuffer = static_cast<D3D12StructuredBuffer*>(
		RenderingAPI::CreateStructuredBuffer(nullptr, Info.ResultDataMaxSizeInBytes, 1, BufferUsage_UAV | BufferUsage_Default));
	if (!ResultBuffer)
	{
		return false;
	}
