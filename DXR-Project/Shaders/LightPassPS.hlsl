...
float4 Main(PSInput Input) : SV_TARGET
{
    ...
    const float		Roughness	= min(max(SampledMaterial.r, MIN_ROUGHNESS), MAX_ROUGHNESS);
    ...
    float3 F_IBL	= FresnelSchlickRoughness(max(dot(Norm, ViewDir), 0.0f), F0, Roughness);
    float3 Ks_IBL	= F_IBL;
    float3 Kd_IBL	= 1.0f - Ks_IBL;
    Kd_IBL *= 1.0 - Metallic;
	
	float2 IntegrationBRDF	= IntegrationLUT.Sample(LUTSampler, float2(max(dot(Norm, ViewDir), 0.0f), Roughness)).rg;
