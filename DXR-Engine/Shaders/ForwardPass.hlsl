...
ConstantBuffer<Material>  MaterialBuffer  : register(b6);
...
float4 PSMain(PSInput Input) : SV_Target0
{
    ...
    float4 DiffuseAlpha  = DiffuseTex.Sample(MaterialSampler, TexCoords);
    float3 SampledAlbedo = ApplyGamma(DiffuseAlpha.rgb) * MaterialBuffer.Albedo;
    ...
    const float3 Specular = SpecularTex.Sample(MaterialSampler, TexCoords).rgb;
    ...
    const float SampledAlpha     = DiffuseAlpha.a;
    ...
    // Emissive
    {
        float3 Emissive = EmissiveTex.Sample(MaterialSampler, TexCoords);
        L0 += Emissive;
    }
