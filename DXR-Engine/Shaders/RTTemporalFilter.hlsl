...
void Main(ComputeShaderInput Input)
{
    ...
    // TODO: Position may not be correct for this pixel
    ...
    int KernelWidth = (int)ceil(lerp(1.0f, 16.0f, min(Roughness, 0.35f) / 0.35f));
    for (int y = 0; y < NUM_SAMPLES; y++)
    {
        ...
        //float2 Rnd = y == 0 ? Float2(0.0f) : trunc(PoissonSamples[y] * KernelWidth);
    ...
    float2 GBufferVelocity = GBuffervelocityTex[TexCoords];
    ...
    HistorySample1.rgb = clamp(HistorySample1.rgb, Min, Max);
