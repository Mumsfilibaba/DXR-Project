...
float PointLightShadowFactor(in TextureCube<float> ShadowMap, in SamplerComparisonState Sampler, float3 WorldPosition, float3 Normal, FShadowPointLight Light, FPositionRadius LightPos)
{
    ...
    const float BiasScale   = 1.0 - saturate(dot(Normal, LightDir));
    const float ShadowBias  = max(max(0.0001, Light.ShadowBias) * BiasScale, 0.0005);
    ...
    {
        const int Index = int(float(NUM_OFFSET_SAMPLES) * Random(floor(WorldPosition.xyz * 1000.0), i)) % NUM_OFFSET_SAMPLES;
